...
public class Physics {
    ...
    private boolean checkLadderTiles(ArrayList<Tile> ladderTiles, ArrayList<Tile> playerTilesOccupying) {
    	boolean ladderUnderneath = false;
    	int tilesUnderneath = 0;
    	
    	for (int i = 0; i < ladderTiles.size(); i++) {
    		for (int j = 0; j < playerTilesOccupying.size(); j++) {
    			if (ladderTiles.get(i).getX() == playerTilesOccupying.get(j).getX() &&
    					ladderTiles.get(i).getY() == playerTilesOccupying.get(j).getY()) {
    				ladderUnderneath = true;
    				tilesUnderneath++;
    				break;
    			}
    		}
    	...
    	return tilesUnderneath > 0;
    }
    
    private ArrayList<Tile> getLadderTiles(Tile[][] interactiveTiles) {
    	ArrayList<Tile> ladderTiles = new ArrayList<Tile>();
    	
    	for (int i = 0; i < interactiveTiles.length; i++) {
    		for (int j = 0; j < interactiveTiles[i].length; j++) {
    			if (interactiveTiles[i][j] instanceof LadderTile) {
    				ladderTiles.add(interactiveTiles[i][j]);
    			}
    		}
    	}
    ...
    private void handleGameObject(LevelObject obj, Level level, int delta){
        ...
        // Check if player can climb a ladder
        if (checkLadderTiles(getLadderTiles(level.getInteractiveTiles()), obj.getBoundingShape().getTilesOccupying(level.getTiles()))) {
        	if (obj instanceof Character) {
        		// Allow climbing for character
        		((Character)obj).allowClimbing(true);
        		
        		// Set 0 vertical force (avoid player to fall)
        		((Character)obj).applyGravity(false, gravity, delta);
        	}
        } else {
        	if (obj instanceof Character) {
        		// Do not allow climbing for character
        		((Character)obj).allowClimbing(false);
        		
        		// Set normal vertical force
        		((Character)obj).applyGravity(true, gravity, delta);
        	}
        }
    }
}
